...
public class Proxy {
    ...
    public Proxy(int port) {
        ...
        if (!System.getProperty("os.name").toLowerCase().contains("linux")) {
            ...
            if (System.getProperty("os.name").toLowerCase().contains("windows")) {
                mitm = "win" + System.getProperty("file.separator") + "mitmdump.exe";
            } else {
                mitm = "mac" + System.getProperty("file.separator") + "mitmdump";
            }
        } else {
    ...
    public void run() {
        List<String> args = new ArrayList<>(Arrays.asList(new String[]{path + mitm, "-p", getPort() + "", "-s", path + "proxy.py"}));
    ...
    public void kill() {
        ...
        if (p != null) {
            if (System.getProperty("os.name").toLowerCase().contains("windows")) {
                try {
                    Runtime.getRuntime().exec("taskkill /f /im mitmdump.exe");
                } catch (IOException ex) {
                    ex.printStackTrace();
                }
            }
